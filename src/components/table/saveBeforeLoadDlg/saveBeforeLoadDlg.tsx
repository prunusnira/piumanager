...
const SaveBeforeLoadDlg = observer(() => {
    ...
    const TxtSaveBeforeLoad =
        language.language === 'ko' ? TxtSaveBeforeLoadKo :
        language.language === 'jp' ? TxtSaveBeforeLoadJp :
        language.language === 'cn' ? TxtSaveBeforeLoadCn : TxtSaveBeforeLoadEn

    return (
        <Modal isOpen={status.status.showSaveBeforeLoadDialog}>
            <ModalHeader>
                {TxtSaveBeforeLoad.title}
            </ModalHeader>
            <ModalBody>
                {TxtSaveBeforeLoad.body.p1}
            </ModalBody>
            <ModalFooter>
                <Button onClick={() => {
