...
type PatternDialogReturn = [
    () => void, (rank: RankType) => void,
    () => void, () => void, (ptid: number, rank: RankType) => void,
]
    ...
    const updateRankCount = () => {
        ...
        for(let i = 0; i < status.status.ptIdList.length; i++) {
            if(user.user.userStatus.has(status.status.ptIdList[i])) {
                ...
                case RankType.SSS: ranksss++; break;
                case RankType.SS: rankss++; break;
                case RankType.S: ranks++; break;
                case RankType.Aon: ranka++; break;
                case RankType.Aoff: rankao++; break;
                case RankType.BCDoff: rankbcdo++; break;
                case RankType.F: rankf++; break;
                case RankType.BCDon: rankbcd++; break;
                }
            }
        }
        ...
        status.status.rankcount.np =
            status.status.ptIdList.length - ranksss - rankss - ranks - ranka - rankao
